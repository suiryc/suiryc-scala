...
package object scala {
  ...
  implicit class RichEnumeration[A <: Enumeration](val en: A) extends AnyVal {

    private type WithAliases = A with EnumerationWithAliases
    private def withAliases: Option[WithAliases] = en match {
    ...
    def byName(s: String): A#Value = withAliases.map(v => v.byName(s): A#Value).getOrElse {
      en.values.find(_.toString.toLowerCase == s.toLowerCase).getOrElse {
